...
在 `templates` 目录下的 YAML 文件模板中，变量（Go template 语法）的值默认在 `nginx/values.yaml` 中定义，只需修改 `nginx/values.yaml` 的内容，即可完成 templates 目录下 YAML 文件的配置。

例如在 `deployment.yaml` 中定义的容器镜像：
...
以上变量值是在 `helm create` 生成 chart 时自动生成的默认值，可根据实际情况修改。
#### 打包和部署 Helm Chart
